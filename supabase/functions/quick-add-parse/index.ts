...
// Keep in sync with packages/shared/src/schemas.ts (edge functions can't
// import workspace packages until we add a bundling step).
...
Return ONLY a JSON object: {"items": [{"name", "category", "quantity", "unit"}], "language"}.
...
- quantity/unit: only if stated or strongly implied; metric units only (g, kg, ml, L, pcs); otherwise null.
...
Deno.serve(async (req) => {
  ...
  let parsed;
  try {
    parsed = quickAddResultSchema.parse(JSON.parse(raw));
  } catch {
